  ...
  const positions = useMemo<ChainPosition[]>(() => {
    const activePositions = encryptedPositions.map(({ target, handle }) => {
      ...
      return {
        ...
        pnl: resolvedPnl,
        href: target.market ? `/market?id=${encodeURIComponent(target.market.slug || target.market.id)}` : "",
        handle,
    ...
    return [
      ...activePositions,
      {
        ...
        pnl: 0,
        href: pendingMarket ? `/market?id=${encodeURIComponent(pendingMarket.slug || pendingMarket.id)}` : "",
        isEncrypted: false,
        isExitPending: true,
